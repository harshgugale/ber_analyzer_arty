...
class top(Module):
	def __init__(self,data_width = 20):
		...
		self.comb += [tx.tx_prbs_config.eq(config),
...
def tb(dut):
	...
	for i in range(10):
		yield
		print("tx: {0:0{1}b} Err: {2} Err place: {3:0{1}b}".format((yield dut.txdata),data_width,(yield dut.bit_error),(yield dut.prbs_error)))
